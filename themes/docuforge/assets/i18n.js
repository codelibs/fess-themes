...
export async function init() {
  locale = pickLocale();
  try {
    const r = await fetch(`/themes/docuforge/i18n/messages.${locale}.json`, { credentials: "same-origin" });
    ...
    if (locale !== "en") {
      try {
        const r2 = await fetch("/themes/docuforge/i18n/messages.en.json", { credentials: "same-origin" });
