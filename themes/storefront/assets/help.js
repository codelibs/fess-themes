...
// Help page renderer for the Storefront SPA.
...
async function fetchHelpContent(locale) {
  const url = `/themes/storefront/help/${locale}.json`;
  try {
    ...
    if (locale !== "en") {
      // Fallback to English.
      const r2 = await fetch("/themes/storefront/help/en.json", { credentials: "same-origin" });
