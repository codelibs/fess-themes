...
// HelpDesk theme i18n loader. Resolves the runtime locale once at boot
...
export async function init() {
  locale = pickLocale();
  try {
    const r = await fetch(`/themes/helpdesk/i18n/messages.${locale}.json`, { credentials: "same-origin" });
    ...
    if (locale !== "en") {
      try {
        const r2 = await fetch("/themes/helpdesk/i18n/messages.en.json", { credentials: "same-origin" });
