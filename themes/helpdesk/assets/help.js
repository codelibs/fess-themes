...
// Help page renderer for the HelpDesk SPA.
...
async function fetchHelpContent(locale) {
  const url = `/themes/helpdesk/help/${locale}.json`;
  try {
    ...
    if (locale !== "en") {
      // Fallback to English.
      const r2 = await fetch("/themes/helpdesk/help/en.json", { credentials: "same-origin" });
