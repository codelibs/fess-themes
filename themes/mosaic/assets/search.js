...
function buildResultCard(d, queryId, order) {
  ...
  h3.appendChild(a);
  // Mosaic: when the hit carries searcher provenance, wrap the title in a
...
async function runSearch() {
  ...
  try {
    ...
    // when configured). Mosaic renders a count-free filter sidebar whose options
...
export function runFromUrl() {
  ...
  // The sidebar's File type group is built from filetype_options, not from facet_views.
  ((api.getConfig() || {}).filetype_options || []).forEach(o => {
    if (o && o.value) facetQueryValues.add("filetype:" + o.value);
  });
  state.exQ = [];
 ...
 * below all of them); otherwise defaultNum().
 */
export function initialNum() {
  const cfg = api.getConfig() || {};
  const fallback = defaultNum();
  let stored = 0;
  try {
...
/**
 * Mosaic: build the count-free, always-present filter groups from
...
function renderActiveChips() {
  ...
  if (Array.isArray(state.facetQueries) && state.facetQueries.length > 0) {
    ...
    // Mosaic: File type sidebar options come from filetype_options (value
