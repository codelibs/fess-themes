...
// Help page renderer for the RawBlock SPA.
...
async function fetchHelpContent(locale) {
  const url = `/themes/rawblock/help/${locale}.json`;
  try {
    ...
    if (locale !== "en") {
      // Fallback to English.
      const r2 = await fetch("/themes/rawblock/help/en.json", { credentials: "same-origin" });
